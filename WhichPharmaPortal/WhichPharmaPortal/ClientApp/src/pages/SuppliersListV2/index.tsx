...
import { columnsArray, columns, columnsAGArray } from './columns';
...
import { goToSuppliers, goToCommunicationConfig } from '../../store/Router/actions';
...
const SuppliersList: React.FC = () => {
    ...
    const searchColumns: string[] = ['name', 'country', 'status', 'contacts', 'classification', 'type', 'notes'];
    ...
    const handleChangePage = React.useCallback(
        (page: number) => dispatch(goToSuppliers(page * tableSettings.pageSize)),
        // eslint-disable-next-line
        [tableSettings.pageSize],
    );
    ...
    const handleFiltersChange = React.useCallback(
        (filters: Filters) => dispatch(goToSuppliers(undefined, filters)),
        [],
    );
    ...
    const handleProceedClick = React.useCallback(
        () => dispatch(goToCommunicationConfig({ Version: query2.Version })),
        [],
    );
