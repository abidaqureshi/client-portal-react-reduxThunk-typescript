...
const generateDefaultSubject = (products: ProductV2[], rfqs: RFQ[]): string => {
    return `RFQ ${rfqs
...
const ProductsEmailTables: React.FC<ProductsEmailTablesProps> = ({
    ...
    let selectedSuppliersEmails = useSelector(getSelectedSuppliersEmails) || [];
    const uniqueEmails = [...new Set([...selectedSuppliersEmails.map((item) => item.value)])];
    //const uniqueEmails = [...selectedSuppliersEmails.map((item) => item.value)];
    const autoFillFields = useSelector(getRFQTablesAutoFillFieldsSettings);
    selectedSuppliersEmails = uniqueEmails.reduce((prev, item) => [{ value: item }, ...prev], [{ value: '' }]);

    const productsByCountry = React.useMemo<MapOf<ProductV2[]>>(() => {
        ...
        return Array.from(new Set(selectedProducts.map((p) => p.countryCode)))
            ...
            //.map((item) => { item.country = item.countryName; return item; })
                        ...
                        table: prev[country]?.table
                            ...
                            .map<RFQQuote>((row) => {
                                ...
                                return {
                                    ...
                                    unitQuant: rfq?.unitQuant || '',
                                    isAlternative: rfq?.isAlternative || false,
                                    numOfPacks:
                                        row.packSize && rfq?.unitQuant
                                            ? Math.ceil(parseInt(rfq?.unitQuant) / parseInt(row?.packSize)).toString()
                                            : '',
                                };
                            }),
                    },
    ...
    return (
        <Panel title={t(TK.productsTables)} subtitle={t(TK.seeAndConfigureTheTablesInEmails)}>
            ...
            <List
                ...
                renderName={(item) => (
                    <CountryFlag
                        country={productsByCountry[item.id][0].countryName}
