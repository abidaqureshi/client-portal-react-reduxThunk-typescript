...
const EmailTemplates: React.FC<EmailContentSetup> = ({ emailTemplates, setEmailTemplates }) => {
    ...
    const countriesNames = React.useMemo<MapOf<string>>(() => {
        ...
        return Array.from(new Set(selectedProducts.map((p) => p.countryCode)))
            .map((countryCode) => ({
                [countryCode]: selectedProducts.find((p) => p.countryCode === countryCode)?.country || countryCode,
            }))
    ...
    React.useEffect(() => {
        setEmailTemplates(
            ['default']
                .concat(Object.keys(countriesNames))
    ...
    const handleEmailTemplateChange = React.useCallback(
        (countryId: string, template: string) => {
                    ...
                    .map<TreeMenuOption>((countryId) => ({
                        ...
                        onClick: () => {
                            setEmailTemplates((prev) => ({
                                ...prev,
                                [countryId]: defaultTemplates[countryId] || defaultTemplates['default'],
                            }));
                            setTab(0);
                        },
                    })),
    ...
    return (
        <Panel title={t(TK.emailContent)} subtitle={t(TK.configureEmailContentMessage)} options={options}>
